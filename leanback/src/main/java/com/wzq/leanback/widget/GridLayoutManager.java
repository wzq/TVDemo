...
final class GridLayoutManager extends RecyclerView.LayoutManager {
     ...
     */
    int mFocusPosition = RecyclerView.NO_POSITION;
    ...
    void dispatchChildSelected() {
        ...
        View view = mFocusPosition == RecyclerView.NO_POSITION ? null : findViewByPosition(mFocusPosition);
        if (view != null) {
            ...
            if (mChildSelectedListener != null) {
                mChildSelectedListener.onChildSelected(mBaseGridView, view, mFocusPosition,
                        vh == null? RecyclerView.NO_ID: vh.getItemId());
            }
            fireOnChildViewHolderSelected(mBaseGridView, vh, mFocusPosition, mSubFocusPosition);
        } else {
            if (mChildSelectedListener != null) {
                mChildSelectedListener.onChildSelected(mBaseGridView, null, RecyclerView.NO_POSITION, RecyclerView.NO_ID);
            }
            fireOnChildViewHolderSelected(mBaseGridView, null, RecyclerView.NO_POSITION, 0);
        }
    ...
    private void dispatchChildSelectedAndPositioned() {
        ...
        if (view != null) {
            ...
            fireOnChildViewHolderSelectedAndPositioned(mBaseGridView, null, RecyclerView.NO_POSITION, 0);
        }
        if (TRACE) TraceCompat.endSection();
    ...
    public boolean canScrollHorizontally() {
        ...
        return mOrientation == RecyclerView.HORIZONTAL || mNumRows > 1;
    }
    ...
    public boolean canScrollVertically() {
        ...
        return mOrientation == RecyclerView.VERTICAL || mNumRows > 1;
    }
    ...
    int getViewPrimarySize(View view) {
        ...
        return mOrientation == RecyclerView.HORIZONTAL ? sTempRect.width() : sTempRect.height();
    }

    private int getViewCenter(View view) {
        return (mOrientation == RecyclerView.HORIZONTAL) ? getViewCenterX(view) : getViewCenterY(view);
    }

    private int getViewCenterSecondary(View view) {
        return (mOrientation == RecyclerView.HORIZONTAL) ? getViewCenterY(view) : getViewCenterX(view);
    }
    ...
    private boolean layoutInit() {
        ...
        if (newItemCount == 0) {
            ...
            mSubFocusPosition = 0;
        } else if (mFocusPosition == RecyclerView.NO_POSITION && newItemCount > 0) {
        ...
        if (!mState.didStructureChange() && mGrid != null && mGrid.getFirstVisibleIndex() >= 0
            ...
            if (!focusViewWasInTree && mFocusPosition != RecyclerView.NO_POSITION) {
        ...
        for (int rowIndex = 0; rowIndex < mNumRows; rowIndex++) {
            ...
            for (int rowItemPairIndex = 0; rowItemPairIndex < rowItemsPairCount;
                ...
                for (int i = rowIndexStart; i <= rowIndexEnd; i++) {
                    ...
                    final int secondarySize = mOrientation == RecyclerView.HORIZONTAL
            ...
            if (!mBaseGridView.hasFixedSize() && measure && rowSize < 0 && itemCount > 0) {
                if (scrapChildWidth < 0 && scrapChildHeight < 0) {
                    int position;
                    if (mFocusPosition == RecyclerView.NO_POSITION) {
                        position = 0;
                ...
                rowSize = mOrientation == RecyclerView.HORIZONTAL ? scrapChildHeight : scrapChildWidth;
            }
            if (rowSize < 0) {
                rowSize = 0;
            }
    ...
    private Grid.Provider mGridProvider = new Grid.Provider() {
        ...
        public int createItem(int index, boolean append, Object[] item) {
            ...
            item[0] = v;
            return mOrientation == RecyclerView.HORIZONTAL ? getDecoratedMeasuredWidthWithMargin(v)
    ...
    void layoutChild(int rowIndex, View v, int start, int end, int startSecondary) {
        ...
        int sizeSecondary = mOrientation == RecyclerView.HORIZONTAL ? getDecoratedMeasuredHeightWithMargin(v)
        ...
        if (mOrientation == RecyclerView.HORIZONTAL && verticalGravity == Gravity.TOP
                || mOrientation == RecyclerView.VERTICAL && horizontalGravity == Gravity.LEFT) {
            // do nothing
        } else if (mOrientation == RecyclerView.HORIZONTAL && verticalGravity == Gravity.BOTTOM
                || mOrientation == RecyclerView.VERTICAL && horizontalGravity == Gravity.RIGHT) {
            startSecondary += getRowSizeSecondary(rowIndex) - sizeSecondary;
        } else if (mOrientation == RecyclerView.HORIZONTAL && verticalGravity == Gravity.CENTER_VERTICAL
                || mOrientation == RecyclerView.VERTICAL && horizontalGravity == Gravity.CENTER_HORIZONTAL) {
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (mFocusPosition != RecyclerView.NO_POSITION && mFocusPositionOffset != Integer.MIN_VALUE) {
        ...
        if (mFocusPosition != RecyclerView.NO_POSITION && scrollToFocus
    ...
    public void collectAdjacentPrefetchPositions(int dx, int dy, State state,
        ...
        try {
            ...
            int da = (mOrientation == RecyclerView.HORIZONTAL) ? dx : dy;
    ...
    public void setSelection(int position, int subposition, boolean smooth,
            int primaryScrollExtra) {
        if (mIsSlidingChildViews || mFocusPosition != position && position != RecyclerView.NO_POSITION
    ...
    public void onItemsAdded(RecyclerView recyclerView, int positionStart, int itemCount) {
        ...
        if (mFocusPosition != RecyclerView.NO_POSITION && mGrid != null && mGrid.getFirstVisibleIndex() >= 0
                && mFocusPositionOffset != Integer.MIN_VALUE) {
    ...
    public boolean onRequestChildFocus(RecyclerView parent, View child, View focused) {
        ...
        if (getPositionByView(child) == RecyclerView.NO_POSITION) {
    ...
    int getScrollOffsetX() {
        return mOrientation == RecyclerView.HORIZONTAL ? mScrollOffsetPrimary : mScrollOffsetSecondary;
    }

    int getScrollOffsetY() {
        return mOrientation == RecyclerView.HORIZONTAL ? mScrollOffsetSecondary : mScrollOffsetPrimary;
    }
    ...
    boolean hasPreviousViewInSameRow(int pos) {
        if (mGrid == null || pos == RecyclerView.NO_POSITION || mGrid.getFirstVisibleIndex() < 0) {
            return false;
        }
    ...
    public boolean onAddFocusables(RecyclerView recyclerView,
        ...
        if (recyclerView.hasFocus()) {
            ...
            final int focusedRow = mGrid != null && focusedPos != RecyclerView.NO_POSITION
                    ? mGrid.getLocation(focusedPos).row : RecyclerView.NO_POSITION;
    ...
    public int getRowCountForAccessibility(RecyclerView.Recycler recycler,
        ...
        if (mOrientation == RecyclerView.HORIZONTAL && mGrid != null) {
    ...
    public int getColumnCountForAccessibility(RecyclerView.Recycler recycler,
            RecyclerView.State state) {
        if (mOrientation == RecyclerView.VERTICAL && mGrid != null) {
            return mGrid.getNumRows();
        }
    ...
    int processSelectionMoves(boolean preventScroll, int moves) {
        ...
        int focusedRow = focusPosition != RecyclerView.NO_POSITION
                ? mGrid.getRowIndex(focusPosition) : RecyclerView.NO_POSITION;
        View newSelected = null;
        for (int i = 0, count = getChildCount(); i < count && moves != 0; i++) {
            ...
            int rowIndex = mGrid.getRowIndex(position);
            if (focusedRow == RecyclerView.NO_POSITION) {

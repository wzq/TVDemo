 ...
 * @deprecated Use {@link com.wzq.leanback.media.MediaControllerGlue}.
 */
@Deprecated
public abstract class MediaControllerGlue extends PlaybackControlGlue {
     ...
     * {@link PlaybackControlGlue.InputEventHandler}
    ...
    public int getCurrentSpeedId() {
        ...
        return PlaybackControlGlue.PLAYBACK_SPEED_INVALID;
    }
    ...
    public long getSupportedActions() {
        ...
        if ((actions & PlaybackStateCompat.ACTION_PLAY_PAUSE) != 0) {
            result |= PlaybackControlGlue.ACTION_PLAY_PAUSE;
        }
        if ((actions & PlaybackStateCompat.ACTION_SKIP_TO_NEXT) != 0) {
            result |= PlaybackControlGlue.ACTION_SKIP_TO_NEXT;
        }
        if ((actions & PlaybackStateCompat.ACTION_SKIP_TO_PREVIOUS) != 0) {
            result |= PlaybackControlGlue.ACTION_SKIP_TO_PREVIOUS;
        }
        if ((actions & PlaybackStateCompat.ACTION_FAST_FORWARD) != 0) {
            result |= PlaybackControlGlue.ACTION_FAST_FORWARD;
        }
        if ((actions & PlaybackStateCompat.ACTION_REWIND) != 0) {
            result |= PlaybackControlGlue.ACTION_REWIND;
        }
        return result;
    }
    ...
    protected void startPlayback(int speed) {
        ...
        if (speed == PlaybackControlGlue.PLAYBACK_SPEED_NORMAL) {
